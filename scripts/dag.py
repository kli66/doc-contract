...
# The `--update` regenerate hint embedded in the roadmap marker points at wherever the skill's
# scripts dir sits in *this* repo (derived, so it stays correct after an install elsewhere).
_SCRIPTS_DIR = Path(__file__).resolve().parent
try:
    _REGEN_PYTHONPATH = _SCRIPTS_DIR.relative_to(REPO_ROOT).as_posix()
except ValueError:  # scripts dir outside the repo root (unusual install) — fall back to absolute
    _REGEN_PYTHONPATH = _SCRIPTS_DIR.as_posix()
DAG_BEGIN = (
    f"<!-- BEGIN GENERATED DAG (regenerate: PYTHONPATH={_REGEN_PYTHONPATH} "
    "python -m dag --update) -->"
)
...
def _infer_persistence(path: Path, declared: str | None) -> str | None:
    parts = path.parts
    if "adr" in parts or "archive" in parts:
        return "frozen"
    return declared


def discover_nodes(root: Path) -> dict[str, Node]:
    nodes: dict[str, Node] = {}
    ...
    for nid, rel in ROOT_NODES.items():
        path = root / rel
        if not path.exists():
...
def _roadmap_text(root: Path) -> str:
    ...
    p = _doc_path(root, "roadmap")
    if not p.exists():
        return ""
    text = p.read_text(encoding="utf-8")
    if DAG_BEGIN in text and DAG_END in text:
        text = text[: text.index(DAG_BEGIN)] + text[text.index(DAG_END) + len(DAG_END) :]
    return text
...
def validate(root: Path, nodes: dict[str, Node], cycle: list[str]) -> list[Finding]:
    findings: list[Finding] = []
    roadmap = _roadmap_text(root)
...
def update_roadmap(root: Path = REPO_ROOT) -> bool:
    ...
    path = _doc_path(root, "roadmap")
    text = path.read_text(encoding="utf-8")
    if DAG_BEGIN in text and DAG_END in text:
        pre = text[: text.index(DAG_BEGIN)]
...
def _main(argv: list[str]) -> int:
    ...
    print(f"\n{len(res.errors)} error(s), {len(res.warnings)} warning(s); {len(res.nodes)} nodes")
